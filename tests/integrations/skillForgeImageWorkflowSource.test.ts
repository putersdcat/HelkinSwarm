...
describe('SkillForge image workflow source wiring', () => {
  it('rebuilds on dev tooling changes and measures startup smoke', () => {
    ...
    expect(workflow).toContain('check_command()');
    expect(workflow).toContain('check_command pnpm');
    expect(workflow).toContain('check_command tsc');
    expect(workflow).toContain('check_command eslint');
    expect(workflow).toContain('check_command prettier');
    expect(workflow).toContain('check_command playwright');
    expect(workflow).toContain('sudo must not be present inside SkillForge container');
    expect(workflow).toContain('docker.sock must not be mounted inside SkillForge container');
    expect(workflow).toContain('docker inspect skillforge-startup-smoke');
