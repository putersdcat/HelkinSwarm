...
describe('deploy-stamp dirty dev mode wiring', () => {
  it('forces the early-dev cost guard through the workflow and asserts the live Azure state after deploy', () => {
    ...
    expect(source).toContain('EARLY_DEV_COST_GUARD is active — guard invariant assertions will run regardless of infraChanged.');
    ...
    expect(source).toContain("Container Apps environment logs destination is '$CAE_DEST' instead of empty/none");
