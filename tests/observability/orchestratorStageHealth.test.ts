...
describe('orchestratorStageHealth', () => {
  ...
  it('tracks active turn stages and exposes age ordering', () => {
    recordOrchestratorStage('corr-1', 'build-prompt', 'user-1', 1_000);
    recordOrchestratorStage('corr-2', 'load-state', 'user-2', 2_000);

    const snapshot = getOrchestratorStageSnapshot(5_000);
  ...
  it('clears completed turn stages', () => {
    recordOrchestratorStage('corr-1', 'llm', 'user-1', 10_000);
    clearOrchestratorStage('corr-1');

    const snapshot = getOrchestratorStageSnapshot(11_000);
