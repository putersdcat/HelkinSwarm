...
df.app.activity('swarmMemoryCommitActivity', {
  handler: async (input: SwarmMemoryCommitInput): Promise<SwarmMemoryCommitResult> => {
    ...
    try {
      ...
      // Helkin's vault only receives synthesis + query; worker research goes to per-agent vaults (#663).
      const highValue = selectHighValueMessages(chatroomTranscript);
      for (const msg of highValue) {
        if (msg.from.toLowerCase() === 'helkin') {
          continue;
        }

        const agentMm = new MemoryManager(userId, msg.from);
        await agentMm.store({
          content: msg.content,
